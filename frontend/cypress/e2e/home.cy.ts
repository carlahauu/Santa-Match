describe('Home Page', () => {
  ...
  it('should render the main image', () => {
    cy.get(
      'img[alt="Santa holding finger to mouth, indicating secret"]'
    ).should('be.visible');
  });
  ...
  it('should render all "How SantaMatch Works" items', () => {
    ...
    cy.get('[data-testid="howItWorksTitle"]')
      .contains('How SantaMatch Works')
      .should('be.visible');
    cy.get('[data-testid="howItWorksItems"] > div').should('have.length', 4);

    items.forEach((item) => {
      cy.get('[data-testid="howItWorksItems"]')
        .contains(item)
        .should('be.visible');
    });
  });
